...
/** 子回复翻页上限（每页 20 条，10 页 ≈ 200 条；匿名仅第一页） */
 ...
 * 获取置顶评论（匿名可用）
 *
 * 「无置顶评论」与「拉取失败」必须可区分——否则上层无法判断是用户真的取消了置顶，
 * 还是接口被限流/对象已删除，会误出无意义的空回顾图并污染 state。
 *
 * @param {string|number} oid 评论对象 ID
 * @param {number} type 对象类型（11=动态 / 1=视频 / 12=文章 / 14=音频）
 * @param {string} [cookie]
 * @param {object} [opts]
 * @param {boolean} [opts.withReason] true 时返回 {comment, reason, upperMid}（不破坏旧签名）
 * @returns {Promise<object|null>} 默认：置顶评论对象；无/不可访问时 null
 */
async function getPinnedComment(oid, type, cookie, opts) {
  const withReason = opts?.withReason ?? false;
  const wrap = (comment, reason, upperMid = null) => (withReason ? { comment, reason, upperMid } : comment);
  let data;
  try {
    ...
    // -404：评论区/对象不存在（如动态已删），与「无置顶评论」语义不同，需上层区分归因
    if (err instanceof BiliError && err.code === -404) return wrap(null, 'notfound');
    // 其他 BiliError（-352/-412 风控等）向上抛：拉取失败不应被当作「已取消置顶」
    throw err;
  }
  const upperMid = data.data?.upper?.mid ?? null;
  const top = data.data?.top_replies?.[0];
  if (!top) return wrap(null, data.data?.replies?.length ? 'none' : 'empty', upperMid);
  const c = top.content || {};
  const comment = {
  ...
  return wrap(comment, 'ok', upperMid);
}

/** 获取置顶评论下的子回复（匿名可用；单页请求，带 web_location 与全量接口保持一致） */
async function getReplies(oid, type, rootRpid, max, cookie) {
  const data = await apiGet(`/x/v2/reply/reply?type=${type}&oid=${oid}&root=${rootRpid}&pn=1&ps=${max}&web_location=333.788`, {
    cookie,
    referer: `https://t.bilibili.com/${oid}`,
  });
...
/** 获取评论目标动态的 UP 信息（评论接口 upper 字段，匿名可用；按 oid 进程内缓存，减少重复请求） */
const _upperCache = new Map(); // `${type}:${oid}` -> {mid,name}|null
const UPPER_CACHE_MAX = 100;
async function getDynamicUpper(oid, type, cookie) {
  const key = `${type}:${oid}`;
  if (_upperCache.has(key)) return _upperCache.get(key);
  const data = await apiGet(`/x/v2/reply?type=${type}&oid=${oid}&sort=2&ps=1`, {
    cookie,
    referer: `https://t.bilibili.com/${oid}`,
  });
  const u = data.data?.upper;
  const res = u ? { mid: u.mid, name: u.name } : null;
  if (_upperCache.size >= UPPER_CACHE_MAX) _upperCache.clear();
  _upperCache.set(key, res);
  return res;
}
/** 测试钩子：清空 UP 信息缓存 */
function _resetUpperCache() { _upperCache.clear(); }
...
async function getAllSubReplies(oid, type, rootRpid, cookie, maxPages = 5) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      await sleep(1000); // 任一页失败：间隔 1s 后重试 1 次（节流一致）
      try {
        data = await apiGet(`/x/v2/reply/reply?type=${type}&oid=${oid}&root=${rootRpid}&pn=${page}&ps=20&web_location=333.788`, {
          cookie,
          referer: `https://t.bilibili.com/${oid}`,
 ...
 * @returns {Promise<{replies: Array, total: number, degraded: boolean, upperMid: (number|string|null)}>}
 */
async function getAllTopCommentsWbi(oid, type, cookie, maxPages = 10) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    upperMid = upperMid ?? (data.upper?.mid ?? null); // 同响应携带 UP mid，省一次 getDynamicUpper 请求
  ...
  return { replies: all, total, degraded: isDegraded(all.length, total), upperMid };
}
...
async function getAllTopCommentsLegacy(oid, type, cookie, maxPages = 10) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    upperMid = upperMid ?? (data.data?.upper?.mid ?? null); // legacy upper.mid 即 UP mid（upper.top 为置顶条目）
    // 终止判定：满足任一即结束
    //  1) 本页不足 ps(20) 条 → 服务端已给完（正常结束）
    //  2) 累计条数达到 page.count 权威总量 → 已拉全
    // 注意：不能只看 count——opus 等类型的 legacy 接口会被降级（只回 3 条但 count 仍报 8809），
    // 此时若按 count 继续翻页会空转多次、徒增风控风险。
    const pageCount = data.data?.page?.count;
    const reachedTotal = Number.isFinite(pageCount) && pageCount > 0 && all.length >= pageCount;
    if (replies.length < 20 || reachedTotal) break;
 ...
 * @returns {Promise<{replies: Array, total: number, degraded: boolean, fallback?: boolean, upperMid: (number|string|null)}>}
 *   fallback=true 表示 wbi 路径不可用（异常/降级）而走了老接口，上层可提示用户
 */
async function getAllTopComments(oid, type, cookie, maxPages = 10) {
  try {
    ...
    return { ...legacy, degraded: legacy.degraded || r.degraded, fallback: true, upperMid: legacy.upperMid ?? r.upperMid };
  } catch (err) {
 ...
 * @returns {Array<{kind:'reply'|'like', parent:any, upReply:any}>}
 */
