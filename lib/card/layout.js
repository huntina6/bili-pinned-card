...
/** 截断提示块（高度预算触顶时追加），返回更新后的 y */
function renderNote(els, y, text) {
  y += 12;
  els.push(`<rect x="${PAD}" y="${y}" width="${CARD_W}" height="42" rx="12" fill="rgba(255,255,255,0.05)"/>`);
  els.push(`<text x="${PAD + 14}" y="${y + 26}" font-size="${SECTION_FS}" fill="${TEXT_DIM}">${esc(text)}</text>`);
  return y + 54;
}

module.exports = {
  defsSvg, renderTitleBar, renderMainCard, renderFooter, renderChainBlock, renderNote,
};
