...
async function checkOnce(cfg) {
  ...
  if (cfg.upTop) {
    ...
    for (const d of dyns) {
      try {
        ...
        const { replies: tops, total: topTotal, degraded, fallback } = await getAllTopComments(d.oid, d.type, cookie);
        if (fallback && !cfg.quiet) {
          log(C.dim(`（wbi 签名接口被限流或不可用，已自动回退老接口拉取评论）`));
        }
        if (degraded && !cfg.quiet) {
          log(C.yellow(`⚠ 评论区可能被 B站 限流（仅获取 ${tops.length}/共 ${topTotal ?? tops.length} 条），结果不完整，建议 --login 刷新 Cookie 后重试`));
        }
  ...
  const { comment, reason: commentReason } = await getPinnedComment(oid, type, cookie, { withReason: true });
