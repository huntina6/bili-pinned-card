...
/** 是否为 B站 动态短链（t.bilibili.com/<dynId> 或 bilibili.com/dynamic/<dynId>，同样需转换） */
function isDynamicLink(v) {
  return /(?:t\.bilibili\.com|bilibili\.com\/dynamic)\/\d+/i.test(String(v || ''));
}

/**
 * 是否为可能的动态 ID 裸数字（19 位，int64 内且不超 B站 动态 ID 段下界）
 * 用途：resolveCommentOid 对裸数字尝试详情转换，与链接输入行为对齐
 * - 评论 oid 为 9-10 位、动态 ID 为 19 位，用长度区分可避免误转
 * - 20 位（App 新 ID 段）超 int64，B站 web 接口必然 -400，不尝试转换
 */
function isBareDynamicId(v) {
  const s = String(v || '').trim();
  return /^\d{15,19}$/.test(s);
}
