...
async function generateUnpinnedIfPossible(st, cfg, reason) {
  ...
  try {
    ...
    const replies = await getAllSubReplies(oldOid, oldType, oldRpid, cfg.cookie, MAX_SUB_PAGES);
    ...
    const { file } = await generateUnpinnedCard({
      comment: detail,
      items,
      opts: { upName: cfg.upName, oid: oldOid, upMid: cfg.uid, scale: cfg.scale },
      outDir: cfg.outDir,
    });
...
async function checkOnce(cfg) {
  ...
  if (cfg.upTop) {
    ...
    for (let di = 0; di < dyns.length; di++) {
      ...
      try {
        if (!cfg.quiet) log(dPrefix + C.dim(`动态 ${d.oid}（${d.author || d.authorMid || ''}）：拉取一级评论...`));
        ...
        let upMid = uid || d.authorMid || upperMid || null;
        if (!upMid) {
          upMid = (await getDynamicUpper(d.oid, d.type, cookie).catch(() => null))?.mid ?? null;
        }
        if (!upMid) {
          if (!cfg.quiet) log(dPrefix + C.dim(`动态 ${d.oid}：无法识别 UP，跳过`));
          continue;
        }
        const upComments = filterUpComments(tops, upMid);
        if (!upComments.length) {
          if (!cfg.quiet) log(dPrefix + C.dim(`动态 ${d.oid}：无 UP 一级评论，跳过`));
          continue;
        }
        if (!cfg.quiet) log(dPrefix + C.dim(`动态 ${d.oid}：UP 一级评论 ${upComments.length} 条，逐条出卡...`));
        for (let ci = 0; ci < upComments.length; ci++) {
          ...
          try {
            ...
            if (existing) {
              skipped++;
              if (!cfg.quiet) log(dPrefix + cPrefix + C.dim(`已存在，跳过 ${path.basename(existing)}（--force 可重出）`));
              continue;
            }
            ...
            if (items.length > MAX_ITEMS_SAFE) {
              ...
              if (!cfg.quiet) log(dPrefix + cPrefix + C.dim(`UP 互动过多，仅展示前 ${MAX_ITEMS_SAFE} 条`));
            }
            // 区域二：粉丝高赞 TOP N
            ...
            cards++;
            if (!cfg.quiet) log(`${dPrefix}${cPrefix}${C.green('✅ UP热评卡:')} ${file}`);
          } catch (err) {
            if (!cfg.quiet) log(dPrefix + cPrefix + C.red(`✗ 评论 ${comment.rpid} 出卡失败: ${err.message}`));
          }
        }
      } catch (err) {
  ...
  if (rpid) {
    ...
    if (context) {
      ...
      const replies = await getAllSubReplies(oid, type || 11, rpid, cookie, MAX_SUB_PAGES);
    ...
    const { file } = await generateCard({
      comment,
      replies,
      opts: { upName: name, upMid: uid, showReplies, oid, scale: cfg.scale },
      outDir,
    });
  ...
  const rpidChanged = !st || st.lastRpid !== (comment ? comment.rpid : '');
  ...
  if (!comment) {
    ...
    if (!trulyUnpinned) {
      if (!cfg.quiet) {
        log(C.yellow(commentReason === 'notfound'
    ...
    saveState(outDir, { ...(st || {}), lastRpid: null, oid, type, lastCheck: new Date().toISOString() });
  ...
  const { file } = await generateCard({
    comment,
    replies,
    // 游客模式（无 uid）时用评论接口同响应的 upper.mid 兜底，保证 UP主 徽标正确
    opts: { upName: name, upMid: uid || pinnedUpperMid, showReplies, oid, scale: cfg.scale },
    outDir,
  });
  saveState(outDir, {
