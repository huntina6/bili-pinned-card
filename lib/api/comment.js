 ...
 * @param {string} [cookie]
 * @param {object} [opts]
 * @param {boolean} [opts.withReason] true 时返回 {comment, reason}（不破坏旧签名）
...
async function getPinnedComment(oid, type, cookie, opts) {
  ...
  const wrap = (comment, reason) => (withReason ? { comment, reason } : comment);
  let data;
  try {
  ...
  if (!top) return wrap(null, data.data?.replies?.length ? 'none' : 'empty');
  const c = top.content || {};
  const comment = {
 ...
 * @returns {Promise<{replies: Array, total: number, degraded: boolean, fallback?: boolean}>}
...
async function getAllTopComments(oid, type, cookie, maxPages = 10) {
  try {
    ...
    return { ...legacy, degraded: legacy.degraded || r.degraded, fallback: true };
  } catch (err) {
