...
const { isOpusLink, isDynamicLink, extractId } = require('./util');
...
async function resolveCommentOid(input, cookie) {
  ...
  if (!isOpusLink(s) && !isDynamicLink(s)) return { oid: extractId(s), type: null }; // 零开销向后兼容
  // 注意：链接分支必须直取 dynId，不能复用 extractId——
  // extractId 的评论 ID 正则最先匹配，若链接带 ?comment_root_id= 会误提取 rpid
  const dynId = s.match(/(?:bilibili\.com\/(?:opus|dynamic)\/|t\.bilibili\.com\/)(\d+)/)?.[1];
