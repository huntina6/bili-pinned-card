...
const { segmentText } = require('./emoji');
...
/** 文本片段 → token（unicode emoji 拆为 emote 图片 token；连续非 emoji 合并为一个文本 token） */
function pushText(tokens, text) {
  let buf = '';
  for (const seg of segmentText(text)) {
    if (seg.emoji) {
      if (buf) { tokens.push({ type: 'text', text: buf }); buf = ''; }
      tokens.push({ type: 'emote', key: seg.text, text: seg.text, unicode: true, url: seg.url });
    } else {
      buf += seg.text;
    }
  }
...
function tokenize(message, emoteMap) {
  ...
  while (i < msg.length) {
    ...
    if (!best) {
      pushText(tokens, msg.slice(i));
      break;
    }
    if (best.idx > i) pushText(tokens, msg.slice(i, best.idx));
...
function wrapTokens(tokens, maxW, fs, emoteW = 36) {
  ...
  for (const t of tokens) {
    if (t.type === 'emote') {
      const w = t.unicode ? Math.round(fs * 1.35) : emoteW + 4;
...
function lineToSvg(line, fs, x0, baseline, emoteImgs) {
  ...
  for (const t of line) {
    if (t.type === 'char') {
      ...
      if (dataUri) {
        const size = t.unicode ? Math.round(fs * 1.25) : t.w - 4;
        out += `<image href="${esc(dataUri)}" x="${x0}" y="${baseline - fs + (fs - size) / 2}" width="${size}" height="${size}"/>`;
        x0 += t.w;
      } else if (t.unicode) {
        buf += t.text; // emoji 图片缺失：回退为原始字符（不显示 [占位]）
...
function truncateTokensToLines(tokens, maxW, fs, maxLines, emoteW = 36) {
  ...
  for (const t of tokens) {
    if (truncated) break;
    if (t.type === 'emote') {
      const ew = t.unicode ? Math.round(fs * 1.35) : emoteW + 4;
      if ((out.length || buf) && curW + ew > maxW) { lines++; curW = 0; if (lines > maxLines) { truncated = true; break; } }
      flush();
      out.push({ ...t });
      curW += ew;
      continue;
