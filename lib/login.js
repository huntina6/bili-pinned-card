...
const { BiliError, httpJson, mergeCookie, anonCookie, UA } = require('./api/client');
...
/**
 * 从 k=v 对中提取认证 Cookie（SESSDATA 等）
 * @param {string[]} pairs 形如 ['SESSDATA=abc', 'bili_jct=xyz', ...]（已去属性）
 */
function extractCookiesFromPairs(pairs) {
  const ck = {};
  for (const kv of pairs) {
    ...
    if (['SESSDATA', 'bili_jct', 'DedeUserID', 'DedeUserID__ckMd5', 'bili_ticket', 'bili_ticket_expires'].includes(k)) {
      ck[k] = v;
    }
  }
  return ck;
...
/**
 * 新版扫码成功的回调 URL 为跨域 ticket 中转（passport.biligame.com/.../crossDomain?ticket=...）：
 * 跟随重定向逐跳收集 Set-Cookie，兑换 SESSDATA 等认证 Cookie。
 * @param {string} url 回调 URL
 * @param {string} [extraCookie] 携带的设备指纹 Cookie（buvid 等）
 */
async function exchangeCrossDomain(url, extraCookie) {
  const headers = {
    'User-Agent': UA,
    'Accept': 'text/html,application/json,*/*',
    'Referer': 'https://account.bilibili.com/', // 实测必需：缺失时服务端不返回 Set-Cookie
  };
  if (extraCookie) headers['Cookie'] = extraCookie;
  const pairs = [];
  let target = String(url);
  for (let hop = 0; hop < 8; hop++) {
    const res = await fetch(target, { headers, redirect: 'manual', signal: AbortSignal.timeout(20000) });
    const sc = typeof res.headers.getSetCookie === 'function' ? res.headers.getSetCookie() : [];
    for (const c of sc) pairs.push(c.split(';')[0]);
  ...
  return extractCookiesFromPairs(pairs);
}

/**
 * 轮询扫码状态直到登录成功
 * @param {string} qrcodeKey
 * @param {{timeoutMs?: number, onStatus?: (code: number, msg: string) => void}} [opts]
 * @returns {Promise<{cookies: Record<string, string>}>} cookies 为认证 Cookie 键值对
 */
  ...
  while (Date.now() < deadline) {
    try {
      const d = await httpJson(
        ...
        { referer: 'https://www.bilibili.com/', throttle: false } // poll 自带 2.5s 间隔，不叠全局节流
      );
      ...
      switch (classifyPoll(inner)) {
        case 'success': {
          ...
          let ck = extractCookiesFromUrl(rawUrl);
          if (!ck.SESSDATA && /ticket=|crossDomain/i.test(rawUrl)) {
            // 新版成功 URL 为 ticket 中转 → 跟随跳转兑换 Cookie
            const device = await anonCookie().catch(() => '');
            ck = await exchangeCrossDomain(rawUrl, device);
          }
 ...
 * @param {{timeoutMs?: number, onStatus?: (code: number, msg: string) => void, log?: (msg: string) => void}} [opts]
