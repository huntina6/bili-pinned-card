 ...
 * @param {string} url
 * @param {{cookie?: string, referer?: string, throttle?: boolean}} [opts]
 */
async function httpJson(url, { cookie, referer, throttle = true } = {}) {
  ...
  if (code === -352 || code === -403 || code === -412 || code === -509 || code === -799) {
    ...
    logger.warn(`风控/签名 code=${code}: ${data.message || ''} @ ${logger.sanitizeUrl(url)}（${ms}ms）`);
  } else if (code === -101) {
...
async function apiGet(urlPath, { cookie, referer } = {}) {
  ...
  if (RISK_CODES.includes(data.code)) {
    ...
    const hint = data.code === -403 ? '接口签名/权限校验失败' : '请求被拦截';
