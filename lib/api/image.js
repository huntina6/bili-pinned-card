...
/**
 * B站 图床格式化参数（docs/misc/picture.md）：`@<w>w_<h>h_<c>c_<q>q.<fmt>`
 * - 仅处理 hdslb/biliimg 域名；w/h 为最大限制（不会放大原图）
 * - crop=true 追加 1c（配合 w+h 裁切为固定比例，常用于头像）
 * - 已带 @ 参数的 URL 只做 webp→jpg 格式替换，不叠加尺寸
 * @param {string} url
 * @param {{w?: number, h?: number, crop?: boolean, quality?: number}} [opts]
 */
function imgVariant(url, { w, h, crop = false, quality } = {}) {
  const s = String(url || '');
  if (!s || !/(hdslb\.com|biliimg\.com)/i.test(s)) return s;
  const qi = s.indexOf('?');
  const pathPart = qi >= 0 ? s.slice(0, qi) : s;
  const query = qi >= 0 ? s.slice(qi) : '';
  const at = pathPart.indexOf('@');
  if (at >= 0) return pathPart.replace(/\.webp$/i, '.jpg') + query;
  const parts = [];
  if (w) parts.push(`${Math.round(w)}w`);
  if (h) parts.push(`${Math.round(h)}h`);
  if (crop) parts.push('1c');
  if (quality) parts.push(`${Math.round(quality)}q`);
  if (!parts.length) return s;
  return `${pathPart}@${parts.join('_')}.jpg${query}`;
}

/** 下载图片（带 UA/Referer，webp 自动转 jpeg；网络/非 2xx 失败重试 1 次，仍失败返回 null） */
async function downloadImage(url) {
  const u = fixWebpUrl(normUrl(url));
