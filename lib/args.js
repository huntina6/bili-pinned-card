...
function parseArgs(argv) {
  const a = {
    ...
    version: false, noInput: false, scale: null, noEmoji: false, width: null,
  };
  const set = (k, v) => { a[k] = v; };
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--max-dyns': {
        const n = parseInt(argValue(argv, i, arg), 10);
      ...
      case '--scale': {
        const n = parseFloat(argValue(argv, i, arg));
      ...
      case '--width': {
        const n = parseInt(argValue(argv, i, arg), 10);
        // 自定义输出宽度（像素），范围 340~4080（=0.5x~6x）；优先于 --scale
      ...
      case '--version': case '-V': set('version', true); break;
...
用法: node cli.js [选项]
  ...
  -o, --out <目录>       输出目录（默认：项目目录下 output）
  --scale <倍率>         渲染倍率（默认 2；0.5~6，支持 1.5 等小数）
  --width <像素>         自定义输出宽度（340~4080，优先于 --scale）
  --no-emoji             关闭 Unicode emoji 彩色化（默认用 Twemoji 内联）
  -q, --quiet            安静模式（仅输出结果行）
