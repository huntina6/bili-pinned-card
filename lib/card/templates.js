...
const {
  ...
  MAX_ITEMS_SAFE, MAX_LINES, maxCardHForScale,
} = require('./constants');
const { esc, fmtCount, fmtTime, measureText, wrapTokens, lineToSvg, toFileTs, truncateTokensToLines } = require('./text');
const { dataUri, picSize, fitSinglePic, prepareImages, prepareChainItems, prepareUpTopCard, picVariant, avatarVariant } = require('./image');
const { defsSvg, renderTitleBar, renderMainCard, renderFooter, renderChainBlock, renderNote } = require('./layout');
...
async function buildSvg(comment, replies, opts = {}) {
  ...
  if (showReplies && replies.length) {
    ...
    for (let i = 0; i < replies.length; i++) {
      if (y > maxH) { cut = replies.length - i; break; }
      const r = replies[i];
      y += 12;
      const rTop = y;
    ...
    if (cut) y = renderNote(els, y, `…还有 ${cut} 条回复未展示（已达卡片高度上限）`);
  }
...
async function buildUnpinnedSvg(comment, items, opts = {}) {
  ...
  if (!items.length) {
    ...
    if (cut) y = renderNote(els, y, `…还有 ${cut} 条互动未展示（已达卡片高度上限）`);
  }
...
async function generateUnpinnedCard({ comment, items, opts, outDir }) {
  ...
  const png = renderPng(svg, opts?.scale || 2);
  fs.mkdirSync(outDir, { recursive: true });
  const ts = toFileTs(comment.ctime) || new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14); // 评论发布时间（兜底生成时间）
...
async function buildUpTopSvg(comment, items, fans, opts = {}) {
  ...
  if (!fans.length) {
    ...
    for (let i = 0; i < fans.length; i++) {
      if (y > maxH) { cut = fans.length - i; break; }
      y = renderChainBlock(els, y, fans[i], '粉丝', false);
    }
    if (cut) y = renderNote(els, y, `…还有 ${cut} 条高赞回复未展示（已达卡片高度上限）`);
  }
...
async function buildDynamicSvg(dyn, opts = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (y + bodyLh > maxH) { cutBody = lines.length - i; break; }
    y += bodyLh;
    els.push(lineToSvg(lines[i], bodyFs, INNER_X, y, {}));
  }
  if (cutBody) y = renderNote(els, y, `…正文还有 ${cutBody} 行未展示（已达卡片高度上限）`);
...
async function generateDynamicCard({ dyn, opts, outDir }) {
  ...
  const png = renderPng(svg, opts?.scale || 2);
  fs.mkdirSync(outDir, { recursive: true });
  const ts = toFileTs(dyn.latestTs) || new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14); // 动态发布时间（兜底生成时间）
